...
with open('vqc/test/_annotations.coco.json', 'r') as f:
...
# Phân loại theo COCO
small = [a for a in areas if a < 32*32]
medium = [a for a in areas if 32*32 <= a < 96*96]
large = [a for a in areas if a >= 96*96]

print(f"Small boxes: {len(small)}")
print(f"Medium boxes: {len(medium)}")
print(f"Large boxes: {len(large)}")
